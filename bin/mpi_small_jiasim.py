...
out_dir = "output/"+expf_name+"_"+str(gradCut)+"_"  # for plots
...
for index in my_tasks:
    print 'index=',index
    print 'shape of sim and data',shape_sim, shape_dat
    imap = lc.get_kappa(index+1,z=1100)
    print 'shape of kappa',imap.shape
    imap_resolution =  np.min(enmap.extent(imap.shape,imap.wcs)/imap.shape[-2:])*60.*180./np.pi
    print 'imap_resolution',imap_resolution
    print enmap.extent(imap.shape,imap.wcs)

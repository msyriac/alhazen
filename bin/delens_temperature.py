...
import argparse
parser = argparse.ArgumentParser(description='Iteratively delens T lensed with GRF')
...
kappa = parray_sim.get_grf_kappa(seed=10,skip_update=False)
...
for k,index in enumerate(my_tasks):
    ...
    kappa_iter_recon = kappa_recon.copy()
    ...
    for j in range(niter):
        ...
        if j==0:
            ...
            wiener = clkk2d*np.nan_to_num(1./(clkk2d+qest.N.Nlkk['TT']))
            wiener[fMask<1.] = 0.
            #wiener /= wiener.max()
